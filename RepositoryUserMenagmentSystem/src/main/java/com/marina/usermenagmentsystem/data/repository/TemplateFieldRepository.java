/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.marina.usermenagmentsystem.data.repository;

import com.marina.usermenagmentsystem.data.model.TemplateField;
import com.marina.usermenagmentsystem.data.model.TemplateFieldPK;
import java.util.List;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

/**
 *
 * @author MARINA
 */
@Repository
public interface TemplateFieldRepository extends JpaRepository<TemplateField, TemplateFieldPK>{
    List<TemplateField> findByTemplateFieldPKTemplateIdFk(Long templateId);
    
}
